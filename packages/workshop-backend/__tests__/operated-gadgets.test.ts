...
describe("gadgetFacetChatId", () => {
  ...
  describe("a pending operated gadget (no head, created in chat 3)", () => {
    ...
    it("resolves the creating chat itself to its own version", () => {
      expect(gadgetFacetChatId(PENDING_OPERATED, 3, true)).toBe(3);
    });

    it("resolves a caller outside any chat to the creating chat", () => {
      expect(gadgetFacetChatId(PENDING_OPERATED, undefined, false)).toBe(3);
    });

    it("resolves a caller in another chat to the creating chat, proposed changes or not", () => {
      expect(gadgetFacetChatId(PENDING_OPERATED, 9, false)).toBe(3);
      expect(gadgetFacetChatId(PENDING_OPERATED, 9, false)).toBe(3);
      expect(gadgetFacetChatId(PENDING_OPERATED, 9, true)).toBe(3);
    });
    ...
    it("goes back to main once the creation is accepted", () => {
      ...
      expect(gadgetFacetChatId({ operateOnly: true, commitId: "a".repeat(40) }, 9, true))
          .toBeUndefined();
  ...
  it("leaves a pending ordinary gadget on the old per-caller rule", () => {
    ...
    expect(gadgetFacetChatId(PENDING, undefined, false)).toBeUndefined();
    expect(gadgetFacetChatId(PENDING, 3, true)).toBe(3);
    expect(gadgetFacetChatId(PENDING, 9, true)).toBe(9);
    expect(gadgetFacetChatId(PENDING, 9, false)).toBeUndefined();
  });
});
...
describe("getGadgetFacetFetcher", () => {
  ...
  async function countAborts(operateOnly: boolean, record: Record<string, unknown> =
    ...
    await withOverseer(async (_instance, impl) => {
      ...
      // Chat 1 proposes changes to gadget 1: a pin is how a chat's proposed changes are derived
      // now (see proposedChangeWorkpieceIds), there is no stored flag.
      impl.storage.chatMeta.put({
        id: 1, title: "Chat", started: new Date(0), lastActive: new Date(0),
        codeBase: { pins: [{ gadgetId: 1, baseCommit: "a".repeat(40) }], generation: 0, revision: 0 },
      });
      ...
      impl.ctx = {
        ...
        // ctx.restore() routes back into [restore]() in the real runtime, which is what reaches
        // the raw facet; stand in for that so this test still measures the aborts live callers
        // cause (see getGadgetFacetFetcher's comment on why every stub is minted this way).
        restore(params: unknown) { return impl.restore(params); },
        facets: {
      ...
      try {
        for (let chatId of callers) await impl.getGadgetFacetFetcher(1, chatId);
      } finally {
        impl.ctx = realCtx;
      }
    });
...
describe("loadGadgetWorker", () => {
  ...
  it("loads a pending operated gadget from its creating chat for a caller in another chat",
    ...
    await withOverseer(async (_instance, impl) => {
      ...
      expect(gadgetFacetChatId(record, 9, false)).toBe(3);  // chat 9 proposes nothing
      expect(gadgetFacetChatId(record, undefined, false)).toBe(3);
