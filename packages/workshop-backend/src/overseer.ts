...
import { Gatekeeper, type GatekeeperSessionActor, HookInitiator, ResourceDescription, ApprovalQueue, ActionDescription, ObservationAuthorizer, ObservationDescription, VendorDescription, SupportedResource, resolveRequestedResource, HookController, HookDescription, ActionKind } from "@gadgets/workshop-shared/gatekeeper";
import {
 ...
 * (committed) version. Three rules, in order:
 *   - Out of chat context there is nothing proposed: main.
 *   - An operated gadget that has a head never runs chat-proposed code (see
 *     GadgetRecord.operateOnly). Every caller therefore resolves to the same version, so callers
 *     in different chats -- the app view, a room agent, a spawned specialist calling back -- can
 *     no longer abort the facet out from under each other to switch versions. A still-*pending*
 *     operated gadget has no main version yet: its files exist only in its creating chat's
 *     proposed changes, so that chat's version is the only one there is.
 *   - Otherwise a chat with no proposed changes is the main version too.
 */
export function gadgetFacetChatId(
    record: Pick<GadgetRecord, "operateOnly" | "commitId">,
    chatId: number | undefined,
    meta: Pick<AiChatMetadata, "hasProposedChanges"> | undefined): number | undefined {
  if (chatId === undefined) return undefined;
  if (record.operateOnly && record.commitId !== undefined) return undefined;
  return meta?.hasProposedChanges ? chatId : undefined;
}
...
class OverseerImpl implements AgentHooks {
  ...
  assertGadgetEditable(gadgetId: WorkpieceId): void {
    if (this.storage.gadgets.get(gadgetId)?.operateOnly) {
  ...
  getGadgetFacetFetcher(gadgetId: WorkpieceId, chatId?: number): Fetcher<DurableObject> {
    ...
    // Decide which version to run. A chat with no proposed changes wants the main-branch facet,
    // not a chat-specific one; an operated gadget always wants the main-branch facet (see
    // gadgetFacetChatId, which is the whole rule).
    chatId = gadgetFacetChatId(
        record, chatId,
        chatId !== undefined ? this.storage.chatMeta.get(chatId) : undefined);
  ...
  async getGadgetFacet(
      gadgetId: WorkpieceId, chatId?: number, actorProfileId?: string): Promise<RpcStub<any>> {
  ...
  async addGatekeeper(
      cls: GatekeeperClass, creationSpec?: GatekeeperCreationSpec,
      caller: GatekeeperCaller = {from: "user"}): Promise<GatekeeperClient<any>> {
    let id = this.allocateWorkpieceId();
  ...
  startGatekeeperSession(
    ...
    switch (target.type) {
      case "gadget": {
        ...
        return this.getGadgetFacet(target.id, chatId, profileId);
      }
  ...
  async ensureOwnerActorAccounts(
    ...
    for (let gk of inScope) {
      if (gk.id in accountChoices) continue;
      let vendorId = observerVendorId(gk);
      if (!vendorId) continue;
      let ids = await owner.listAccountIdsForVendor(vendorId);
...
class OverseerClientInterface extends RpcTarget implements Overseer {
  ...
  async createGadget(title: string, chatId?: number, bindingName?: string)
    ...
    return new GadgetClientImpl(this.impl, record.id, this.clientUserId, this.clientProfileId);
  }
  ...
  async getGadget(id: WorkpieceId): Promise<RpcStub<GadgetClient>> {
    ...
    // @ts-expect-error An RpcTarget implementing the interface works in place of a stub, but the
    //     type system doesn't know this.
    return new GadgetClientImpl(this.impl, id, this.clientUserId, this.clientProfileId);
  }
  ...
  async newGatekeeper(accountId: number, resourceUrl: string)
    ...
    let result = await this.impl.addGatekeeper(
        cls, creationSpec, {from: "user", profileId: this.clientProfileId});
...
class GadgetClientImpl extends RpcTarget implements GadgetClient {
  ...
  async connectToGadget(chatId?: number): Promise<RpcStub<any>> {
    ...
    return this.impl.getGadgetFacet(this.id, chatId, this.clientProfileId);
  }
...
class UseGadgetClientInterface extends RpcTarget implements GadgetClient {
  ...
  async connectToGadget(chatId?: number): Promise<RpcStub<any>> {
    ...
    return this.impl.getGadgetFacet(this.id, undefined, this.clientProfileId);
  }
